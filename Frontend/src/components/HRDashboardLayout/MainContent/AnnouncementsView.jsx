...
const AnnouncementsView = () => {
  ...
  const [targetAudience, setTargetAudience] = useState('employee'); // Default employee targeting for HR
  ...
  const loadData = async () => {
    try {
      ...
      if (targetResponse.data) {
        ...
        // Filter out admins from recipient choices for HR dashboard targeting
        const nonAdmins = (targetResponse.data.users || []).filter(u => u.role.toLowerCase() !== 'admin');
        setUsers(nonAdmins);
        ...
        if (nonAdmins.length > 0) {
          setTargetRecipient(nonAdmins[0].id);
        }
      }
    } catch (err) {
      console.error('Error loading announcement dashboard data for HR:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      setFormSuccess('Announcement successfully published & broadcasted to Employees!');
      setTitle('');
      setDescription('');
        ...
        <div className={styles.chartContainer} style={{ flex: 1 }}>
          ...
          <form onSubmit={handlePublish} style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '14px' }}>
            ...
            <div className={styles.inputGroup}>
              ...
              <input 
                type="text" 
                placeholder="e.g., Department Policy Update" 
                value={title}
